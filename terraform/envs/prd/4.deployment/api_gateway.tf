...
resource "aws_api_gateway_integration" "status_get" {
  ...
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.screenshot_status_checker.invoke_arn
}
...
resource "aws_lambda_permission" "api_gateway_status_checker" {
  ...
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.screenshot_status_checker.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_api_gateway_rest_api.screenshot_api.execution_arn}/*/*"
}
...
resource "aws_api_gateway_deployment" "screenshot_api" {
  depends_on = [
    aws_api_gateway_integration.screenshot_post,
